...
export const BulkEmailOptions = ({
                                     tenants, apartments, register = () => {
    }, errors, ...restOfProps
                                 }) => {
        ...
        <>
            ...
            <div onClick={() => changeSelected(() => setSelectedTenantsChecked(true))}>
                <FormCheck className="mb-3" style={{display: "block"}} {...register(`recipients`, {
                    required: true,
                    setValueAs: value => value !== null ? value.toString() : ""
                })} type="radio" inline value={"Tenants"} id={"Tenants"} label="Choose Tenants from list"
                           checked={selectedTenantsChecked}/>
            </div>
            <Form.Group as={Col} xs={4} className="mb-3" controlId="selected_recipients"
                        hidden={!selectedTenantsChecked}>
                ...
                <Form.Select multiple value={selectedTenants} htmlSize={tenants.length < 20 ? tenants.length : 20}
                    ...
                    {tenants.map(tenant => <option value={tenant.user_id}
                                                   key={`${tenant.semester}_${tenant.user_id}`}>{tenant.name}</option>)}
                </Form.Select>
                {errors && errors.selected_recipients && <Form.Text
                    className={classNames("text-danger")}>{errors && errors.selected_recipients.message}</Form.Text>}
            </Form.Group>
            <div onClick={() => changeSelected(() => setSelectedApartmentsChecked(true))}>
                <Form.Check className="mb-3" style={{display: "block"}} {...register(`recipients`, {
                    required: true,
                    setValueAs: value => value !== null ? value.toString() : ""
                })} type="radio" inline value={"Apartments"} id={"Apartments"}
                            label="All Tenants of selected apartments" checked={selectedApartmentsChecked}/>
            </div>
            <Form.Group as={Col} xs={4} className="mb-3" controlId="selected_apartments"
                        hidden={!selectedApartmentsChecked}>
                <Form.Label visuallyHidden={true}>Recipient List</Form.Label>
                <Form.Select multiple value={selectedApartments}
                             htmlSize={apartments.length < 20 ? apartments.length : 20}
                    ...
                    {apartments.map(apartment => <option value={apartment.apartment_number}
                                                         key={`${apartment.semester}_${apartment.apartment_number}`}>{apartment.apartment_number}</option>)}
                </Form.Select>
                {errors && errors.selected_apartments && <Form.Text
                    className={classNames("text-danger")}>{errors && errors.selected_apartments.message}</Form.Text>}
            </Form.Group>
        </>
    )
};
