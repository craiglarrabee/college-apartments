...
const Assignments = ({
    ...
    const handleDragEnd = async ({active, over, ...restOfProps}) => {
        ...
        tenant.apartment_number = over.id;
        tenant.room_type_id = over.data?.current?.roomTypeId;
        tenant.room_type = currentLeaseRoomTypes.find(type => type.room_type_id === tenant.room_type_id)?.room_type;
    ...
    const deletePreviousTenantData = async (userId, leaseId) => {
        ...
        console.log(`${new Date().toISOString()} -` +`Application and lease were deleted for user: ${userId} and lease: ${leaseId} in assignments[all].deletePreviousTenantData.`);
        delete tenant.spots;
    ...
    const setPreviousTenantData = async (data) => {
        ...
        const formattedDate = new Intl.DateTimeFormat("en-US", { month: "long", day: "2-digit", year: "numeric" }).format(submit_date);
        data.site = site;
        data.created_by_user_id = user.id;
        data.submit_date = submit_date.toISOString();
        data.processed = true;
        data.newApp = true;
