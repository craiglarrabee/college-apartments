...
const Home = ({site, navPage, links, user, tenant, currentLeases, application}) => {
    ...
    const {
        ...
        handleSubmit
    } = useForm({defaultValues: {...tenant, ...application}});
    ...
    const onSubmitApplication = async (data, event) => {
        ...
        data.processed = !application.processed;

        //if modifications have been made, we POST and just update the application
        //otherwise we PUT which will mark the existing application processed or unprocessed
                    ...
                    <Tabs defaultActiveKey={1}>
                        ...
                        <Tab title="Application" eventKey={2}>
                            <Form onSubmit={handleSubmit(onSubmitApplication)} method="post">
                                {site === "suu" ? <WorkFormGroups register={register} application={application}
                                                                  errors={errors}/> : null}
                                ...
                                <div style={{width: "100%"}}
                                    ...
                                    <Button variant="primary"
                                            type="submit">{isDirty ? "Save" : application.processed ? "Mark Unprocessed" : "Mark Processed"}</Button>
                                </div>
                            </Form>
                        </Tab>
                    </Tabs>
