...
describe("UnprocessedApplicationList component", () => {
    ...
    it("should render the users names", () => {
        const {getByText} = render(<UnprocessedApplicationList data={users} site={site} page={page} leaseId={leaseId} />);
    ...
    it("delete button should call handleDelete function", async () => {
        const delFn = jest.fn();
        const {getByText} = render(<UnprocessedApplicationList data={[users[0]]} handleDelete={delFn} site={site} page={page} leaseId={leaseId} /> );
        await act(() => user.click(getByText("Delete")));
        await waitFor(() => {expect(delFn).toHaveBeenCalledWith(users[0].user_id, site, leaseId)});
    });

    it("process button should call handleProcess function", async () => {
        const procFn = jest.fn();
        const {getByText} = render(<UnprocessedApplicationList data={[users[0]]} handleProcess={procFn} site={site} page={page} leaseId={leaseId} /> );
        await act(() => user.click(getByText("Process")));
        await waitFor(() => {expect(procFn).toHaveBeenCalledWith(users[0].user_id, site, leaseId, true)});
    });

    it("user's name should have href to their information", async () => {
        const {getByText} = render(<UnprocessedApplicationList data={[users[0]]} site={site} page={page} leaseId={leaseId} /> );
...
describe("ProcessedApplicationList component", () => {
    ...
    it("delete button should call handleDelete function", async () => {
        const delFn = jest.fn();
        const {getByText} = render(<ProcessedApplicationList data={[users[0]]} handleDelete={delFn} site={site} page={page} leaseId={leaseId} /> );
        await act(() => user.click(getByText("Delete")));
        await waitFor(() => {expect(delFn).toHaveBeenCalledWith(users[0].user_id, site, leaseId)});
    });
    ...
    it("deposit button should call handleDeposit function", async () => {
        const depFn = jest.fn();
        const {getByText} = render(<ProcessedApplicationList data={[users[0]]} handleDeposit={depFn} site={site} page={page} leaseId={leaseId} /> );
        await act(() => user.click(getByText("Deposit")));
        await waitFor(() => {expect(depFn).toHaveBeenCalledWith(users[0].user_id, site, leaseId)});
    });

    it("process button should call handleUnprocess function", async () => {
        const unprocFn = jest.fn();
        const {getByText} = render(<ProcessedApplicationList data={[users[0]]} handleProcess={unprocFn} site={site} page={page} leaseId={leaseId} /> );
        await act(() => user.click(getByText("Unprocess")));
        await waitFor(() => {expect(unprocFn).toHaveBeenCalledWith(users[0].user_id, site, leaseId, false)});
    });

    it("user's name should have href to their information", async () => {
        const {getByText} = render(<ProcessedApplicationList data={[users[0]]} site={site} page={page} leaseId={leaseId} /> );
        const url = getByText(users[0].name).href;
        expect(url.endsWith(`${page}/${users[0].user_id}?site=${site}`)).toBe(true);
    });
});
