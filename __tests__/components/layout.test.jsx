...
describe("Layout component", () => {
    const mockSite = "test-site";

    test("renders children", () => {
        const {getByText} = render(<Layout site={mockSite}>Test Content</Layout>);
        const content = getByText(/Test Content/i);
        expect(content).toBeInTheDocument();
    });

    test("applies default classnames to main wrapper", () => {
        const {container} = render(<Layout site={mockSite}>Test Content</Layout>);
    ...
    test("does not apply undefined classnames to main wrapper", () => {
        const {container} = render(<Layout site={mockSite} bg={undefined}>Test Content</Layout>);
        const mainWrapper = container.querySelector(".main-wrapper");
