...
export const GetNavLinks = async (user, site) => {
    ...
    if (user && user.admin.includes(site) && user.manageApartment) {
        ...
        queryString = "SELECT site, IF((parent_page = 'leases_'), CONCAT(page, '/manage'), page) AS page, parent_page, position, label, sub_menu, target, unrestricted, restricted, maintainable FROM site_nav WHERE site = ? AND restricted = 1 AND page != 'leases' UNION " +
            ...
            "UNION\n" +
            "SELECT DISTINCT n.site, CONCAT('assignments/', l.fall_year, '/fall/manage') AS page,\n" +
            "       REPLACE(n.parent_page, 'leases', 'assignments') AS parent_page, concat('00', n.position, '3') AS position,\n" +
            ...
            "JOIN user_lease_tenant ul ON ul.lease_id = l.id AND ul.fall_year = l.fall_year \n" +
            "JOIN application a on a.lease_id = ul.lease_id AND a.user_id = ul.user_id AND a.processed = 1 AND a.deposit_date IS NOT NULL \n" +
            "WHERE n.site = ? AND n.page LIKE 'leases%'\n" +
            "UNION\n" +
            "SELECT DISTINCT n.site, CONCAT('assignments/', l.spring_year, '/spring/manage') AS page,\n" +
            "       REPLACE(n.parent_page, 'leases', 'assignments') AS parent_page, concat('00', n.position, '1') AS position,\n" +
            "       CONCAT('Spring ', l.spring_year) AS label,\n" +
            "       n.sub_menu, n.target, n.unrestricted, n.restricted, n.maintainable\n" +
            "FROM site_nav n \n" +
            "JOIN lease l ON l.site = n.site AND l.id = SUBSTR(n.page, LOCATE('/', n.page) + 1) AND l.spring_year >= YEAR(curdate())\n" +
            "JOIN user_lease_tenant ul ON ul.lease_id = l.id AND ul.spring_year = l.spring_year \n" +
            "JOIN application a on a.lease_id = ul.lease_id AND a.user_id = ul.user_id AND a.processed = 1 AND a.deposit_date IS NOT NULL \n" +
            "WHERE n.site = ? AND n.page LIKE 'leases%'\n" +
            "UNION\n" +
            "SELECT DISTINCT n.site, CONCAT('assignments/', l.summer_year, '/summer/manage') AS page,\n" +
            "       REPLACE(n.parent_page, 'leases', 'assignments') AS parent_page, concat('00', n.position, '2') AS position,\n" +
            ...
            "JOIN user_lease_tenant ul ON ul.lease_id = l.id AND ul.summer_year = l.summer_year \n" +
            "JOIN application a on a.lease_id = ul.lease_id AND a.user_id = ul.user_id AND a.processed = 1 AND a.deposit_date IS NOT NULL \n" +
            "WHERE n.site = ? AND n.page LIKE 'leases%'\n" +
            "ORDER BY position, page\n";
        params = [site, site, site, site, site, site];
    } else if (user && user.admin.includes(site) && user.editSite) {
        ...
        queryString = "SELECT n.* FROM site_nav n WHERE n.site = ? AND ( n.unrestricted = 1 OR n.page = 'leases_') UNION " +
            ...
            "       REPLACE(n.parent_page, 'leases', 'assignments') AS parent_page, concat('00', position, '3') AS position,\n" +
            "       CASE WHEN l.fall_year IS NOT NULL THEN CONCAT('Fall ', l.fall_year) WHEN l.spring_year IS NOT NULL THEN CONCAT('Spring ', l.spring_year) WHEN l.summer_year IS NOT NULL THEN CONCAT('Summer ', l.summer_year) END AS label,\n" +
            "       n.sub_menu, n.target, n.unrestricted, n.restricted, n.maintainable\n" +
            "FROM site_nav n JOIN lease l ON l.site = n.site AND l.id = SUBSTR(n.page, LOCATE('/', n.page) + 1) AND l.fall_year >= YEAR(curdate()) - 1\n" +
            ...
            "       REPLACE(n.parent_page, 'leases', 'assignments') AS parent_page, concat('00', position, '1') AS position,\n" +
            "       CONCAT('Spring ', l.spring_year) AS label,\n" +
            "       n.sub_menu, n.target, n.unrestricted, n.restricted, n.maintainable\n" +
            ...
            "       REPLACE(n.parent_page, 'leases', 'assignments') AS parent_page, concat('00', position, '2') AS position,\n" +
            "       CONCAT('Summer ', l.summer_year) AS label,\n" +
            "       n.sub_menu, n.target, n.unrestricted, n.restricted, n.maintainable\n" +
            "FROM site_nav n JOIN lease l ON l.site = n.site AND l.id = SUBSTR(n.page, LOCATE('/', n.page) + 1) AND l.summer_year >= YEAR(curdate())\n" +
            "JOIN user_lease ul ON ul.lease_id = l.id AND ul.user_id = ? " +
