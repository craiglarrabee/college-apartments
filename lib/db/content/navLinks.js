...
export const GetNavLinks = async(site, includeRestricted) => {
    ...
    if (includeRestricted) {
        queryString += "UNION SELECT n.* FROM site_nav n LEFT JOIN lease_definition d ON d.site = n.site AND n.page = CONCAT('leases/', d.id) WHERE n.site = ? AND n.restricted = 1 AND COALESCE(d.end_date, CURDATE() + interval 1 month) >= CURDATE() ";
        params.push(site);
    }
