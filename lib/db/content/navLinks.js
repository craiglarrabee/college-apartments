...
export const GetNavLinks = async (user, site) => {
    ...
    if (user?.manage?.includes(site) && user?.manageApartment) {
        ...
        queryString = `
            SELECT n.site,
                   IF((n.parent_page = 'leases_'), CONCAT(n.page, '/manage'), n.page) AS page,
                   n.parent_page,
                   n.position,
                   n.label,
                   n.sub_menu,
                   ...
                   n.public
            FROM site_nav n
                     LEFT JOIN lease d ON d.site = n.site AND n.page = CONCAT('leases/', d.id) 
            WHERE n.site = ? 
              AND n.restricted = 1 
              AND COALESCE(d.end_date, CURDATE() + interval 12 month) >= CURDATE()
              AND n.page != 'leases'
            UNION
            -- we duplicate the leases under applications to get the same names
            SELECT n.site,
                   REPLACE(IF((n.parent_page = 'leases_'), CONCAT(n.page, '/manage'), n.page), 'leases',
                           'applications')                        AS page,
                   REPLACE(n.parent_page, 'leases', 'applications') AS parent_page,
                   concat('0', n.position)                          AS position,
                   REPLACE(n.label, 'Leases', 'Applications')       AS label,
                   n.sub_menu,
                   n.target,
                   n.unrestricted,
                   n.restricted,
            ...
            SELECT DISTINCT n.site,
                            CONCAT('assignments/', REPLACE(ult.semester, ' ', '_'),
                                   '/manage${site === "suu" ? "/location" : ""}') AS page,
            ...
            FROM site_nav n
                     ...
                     JOIN user_lease_tenant ult ON ult.lease_id = l.id AND
                                                   ...
                                                   YEAR(curdate()) - 2
                     JOIN application a on a.lease_id = ult.lease_id AND a.user_id = ult.user_id AND a.processed = 1
            WHERE n.site = ?
              AND n.page LIKE 'leases%'
              AND COALESCE(l.end_date, CURDATE() + interval 12 month) >= CURDATE()
        ...
        queryString = `
            ...
            FROM site_nav n
                     JOIN lease l ON l.site = n.site AND l.id = SUBSTR(n.page, LOCATE('/', n.page) + 1) AND
                                    SUBSTR(l.semester1, LOCATE(' ', l.semester1) + 1) >= YEAR(curdate()) - 2
                     JOIN user_lease ul ON ul.lease_id = l.id AND ul.user_id = ?
            ORDER BY position, page
        `;
        params = [site, user.id, site, site, user.id, site];
    } else {
        queryString = "SELECT n.* FROM site_nav n WHERE n.site = ? AND n.public = 1 ORDER BY position";
        params = [site];
    }

    let [rows] = await ExecuteQuery(queryString, params);
    if (!user?.manage?.includes(site) && !user?.processedSites?.includes(site)) {
        // filter out payments link if it shouldn't be displayed
        rows = rows.filter(link => link.page !== "payments")
    }
    return rows;
}
