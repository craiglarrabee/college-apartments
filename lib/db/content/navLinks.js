...
export const GetNavLinks = async(user, site) => {
    ...
    if(user && user.admin === site && user.manageApartment) {
        ...
        queryString = "SELECT n.* FROM site_nav n WHERE n.site = ? AND ( n.restricted = 0 OR n.page = 'leases_') UNION SELECT n.* FROM site_nav n JOIN user_lease l ON n.page = CONCAT('leases/', l.lease_id) AND l.user_id = ? WHERE n.site = ? ORDER BY position";
