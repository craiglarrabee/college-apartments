    ...
    if (user && user.admin.includes(site) && user.manageApartment) {
        ...
        queryString = `
            ...
            SELECT DISTINCT n.site,
                            CONCAT('assignments/', REPLACE(ult.semester, ' ', '_'), '/manage') AS page,
        ...
        queryString = `
            ...
            FROM site_nav n
            WHERE n.site = ?
              AND (n.unrestricted = 1 OR n.page = 'leases_')
            UNION
            SELECT n.*
            FROM site_nav n
                     JOIN user_lease l ON n.page = CONCAT('leases/', l.lease_id) AND l.user_id = ?
            WHERE n.site = ?
            UNION
            -- we get assignments links based on semesters
            SELECT site,
