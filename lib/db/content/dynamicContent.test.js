...
describe("GetDynamicContent", () => {
    ...
    it("should return rows", async () => {
        ...
        // Current implementation selects site,page,name,content and uses page LIKE ?
        const [sql, params] = ExecuteQuery.mock.calls[0];
        expect(sql).toEqual(expect.stringContaining("FROM site_content"));
        expect(sql).toEqual(expect.stringContaining("WHERE site = ?"));
        expect(sql).toEqual(expect.stringContaining("page"));
        expect(params).toEqual(["test-site", "test-page"]);
