...
export const GetUserPayments = async (site, userId) => {
    const query = `
        ...
        FROM user_payment p
        WHERE p.site = ? AND p.user_id = ? AND p.date_deleted IS NULL
...
export const GetUserDeletedPayments = async (site, userId) => {
    const query = `
        SELECT p.id, p.user_id, DATE_FORMAT(p.date, '%m/%d/%Y') AS date, p.amount, p.surcharge, p.total, p.description, 
               p.location, DATE_FORMAT(p.date_deleted, '%m/%d/%Y') AS date_deleted, p.reason_deleted,
               p.account_type, p.account_number, p.trans_id, p.auth_code, p.result_code, p.result_message
        FROM user_payment p
        WHERE p.site = ? AND p.user_id = ? AND p.date_deleted IS NOT NULL
        ORDER BY trans_id desc
    `;
    let [rows] = await ExecuteQuery(query,
        [
...
export const MarkPaymentDeleted = async (id, reason) => {
    let [rows] = await ExecuteQuery(
        `UPDATE user_payment
         SET date_reviewed = NOW(), date_deleted = NOW(), reason_deleted = ?
         WHERE id = ?`,
        [
            reason,
            id
