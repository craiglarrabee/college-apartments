...
export const GetApplicationInfo = async (site, userId, leaseId) => {
    ...
    let [rows] = await conn.execute("SELECT * FROM application WHERE site = ? AND user_id = ? AND lease_id = ?", [site, userId, leaseId]);
...
export const GetPendingApplicationInfo = async (userId) => {
    const conn = Connection();
    let [rows] = await conn.execute("SELECT * FROM application WHERE user_id = ? AND processed = false", [userId]);
    //we should only have one
    if (rows && rows.length === 1) return rows[0];
        return null;
}
