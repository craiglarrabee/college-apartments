...
export const GetApplications = async (site, leaseId, processed) => {
    const [rows] = await ExecuteQuery("SELECT t.user_id, CONCAT(t.first_name, ' ', t.last_name) as name, DATE_FORMAT(a.submit_date, '%M %d, %Y') AS submit_date FROM application a INNER JOIN tenant t ON a.user_id = t.user_id WHERE a.site = ? AND a.lease_id = ? AND a.processed = ?", [site, leaseId, processed ? 1 : 0]);
    return rows;
}
...
export const GetPendingApplication = async (userId) => {
