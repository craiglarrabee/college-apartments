...
export const GetTenantUserLeases = async (site, userId, direction="ASC") => {
    let [rows] = await ExecuteQuery(`SELECT DISTINCT l.id,
                                                     ...
                                                     l.lease_parent_phone,
                                                     CONCAT(t.first_name, ' ', t.last_name)  AS name,
                                                     COALESCE(n.label, le.description)       AS label,
                                                     lr.room_rent,
                                                     t.apartment_number
                                     FROM user_lease l
                                              ...
                                              INNER JOIN lease le ON le.id = l.lease_id
                                              LEFT JOIN site_nav n on n.page = CONCAT('leases/', l.lease_id) AND n.site = a.site
...
export const AddUserLease = async (userId, leaseId, data) => {
    ...
    try {
        const [result] = await ExecuteQuery(`
                    REPLACE INTO user_lease (lease_id, user_id, room_type_id, lease_address, lease_home_phone,
                    ...
                    WHERE a.lease_id = ?
                      ...
                      `,
            [
                data.discount !== null && data.discount !== undefined ? data.discount : 0,
                leaseId,
                userId
            ]);
        console.log(`${new Date().toISOString()} -` +`Added lease: ${leaseId} for user: ${userId}. Affected rows: ${result.affectedRows}`);
    } catch (e) {
        console.error(`${new Date().toISOString()} -` , e);
        console.error(`${new Date().toISOString()} -` +`Unable to add lease: ${leaseId} for user: ${userId}`);
    }
}
...
export const DeleteUserLease = async (userId, leaseId, roomTypeId) => {
    ...
    try {
        ...
        console.log(`${new Date().toISOString()} -` +`Deleted lease: ${leaseId} for user: ${userId}`);
    } catch (e) {
        console.error(`${new Date().toISOString()} -` , e);
        console.error(`${new Date().toISOString()} -` +`Unable to delete lease: ${leaseId} for user: ${userId}`);
    }
}
...
export const UpdateUserLease = async (userId, leaseId, data) => {
    try {
        let signDate;
        try {
            signDate = !data.signed_date ? null : new Date(data.signed_date).toISOString();
        } catch (e) {
            signDate = null;
        }
        await ExecuteTransaction([
...
export const SetWelcomeDate = async (userId, leaseId, data) => {
    try {
        await ExecuteTransaction([
            {
                // update the lease with user provided data
                string: `
                    UPDATE user_lease
                    SET welcome_date=NOW(), signed_date = NULL, signature = NULL, lease_discount=?
                    WHERE lease_id = ?
                      AND user_id = ?`
                ,
                params:
...
export const GetActiveSemesterTenantNames = async (site) => {
    const query = `
        ...
        WHERE l.site = ?
          AND SUBSTR(ult.semester, LOCATE(' ', ult.semester) + 1) >= (YEAR(CURDATE()) - 2)
...
export const GetActiveSemesterTenants = async (site) => {
    const query = `
        SELECT DISTINCT ult.semester                                                      AS "Semester",
                        ...
                        CASE WHEN ult_old.lease_id IS NULL THEN 'Returning' ELSE 'New' END AS "Tenant Status",
                        CASE rt.room_type
                        ...
                        CASE WHEN ul.signed_date IS NULL THEN 'Not Signed' ELSE 'Signed' END AS "Lease Agreement Status",
...
export const GetMostRecentTenantApartment = async (site, userId) => {
    // Returns minimal identity and apartment info from the most recent lease record for this user/site
    // Used by the maintenance form to auto-populate hidden fields and apartment number.
    const query = `
        SELECT t.apartment_number,
               t.first_name,
               t.last_name,
               t.email,
               u.username
        FROM user_lease_tenant t
                 INNER JOIN application a ON t.user_id = a.user_id AND t.lease_id = a.lease_id
                 INNER JOIN user u ON u.id = t.user_id
        WHERE a.site = ?
          AND t.user_id = ?
        ORDER BY t.lease_id DESC
        LIMIT 1
    `;
