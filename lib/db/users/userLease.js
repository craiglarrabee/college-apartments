...
export const UpdateUserLease = async (userId, leaseId, data) => {
    try {
        await ExecuteTransaction([
            ...
            {
                string: `
                    INSERT INTO user_required_payments (user_id, lease_id, payment_number, payment_type, amount)
                    SELECT DISTINCT ult.user_id, ult.lease_id, 1, 'payment', (lr.room_rent - ul.lease_discount) / 2
                    ...
                    WHERE ult.user_id = ?
                      AND ult.lease_id = ?
                    UNION
                    SELECT DISTINCT ult.user_id, ult.lease_id, 2, 'payment', (lr.room_rent - ul.lease_discount) / 2
                    FROM user_lease_tenant ult
                             JOIN user_lease ul on ult.lease_id = ul.lease_id AND ult.user_id = ul.user_id
                             JOIN lease_rooms lr on lr.lease_id = ul.lease_id AND lr.room_type_id = ul.room_type_id
                    WHERE ult.user_id = ?
