...
export const GetTenantUserLeases = async (userId) => {
...
export const UpdateUserLease = async (userId, leaseId, data) => {
    try {
        await ExecuteTransaction([
            ...
            {
                // setup the required payments
                string: `
                    INSERT INTO user_required_payments (user_id, lease_id, semester, payment_number, payment_type, amount)
                    SELECT DISTINCT ult.user_id, ult.lease_id, ult.semester, pn.number, 'payment', (lr.room_rent - ul.lease_discount) / 2
                    FROM user_lease_tenant ult
                             JOIN user_lease ul on ult.lease_id = ul.lease_id AND ult.user_id = ul.user_id
                             JOIN lease_rooms lr on lr.lease_id = ul.lease_id AND lr.room_type_id = ul.room_type_id
                    JOIN (SELECT 1 AS number UNION SELECT 2 ) pn
                    WHERE ult.user_id = ?
                      AND ult.lease_id = ?
                      AND ult.semester IS NOT NULL 
                `,
                params:
                    [
                        userId,
...
export const GetActiveSemesters = async (site) => {
    const query = `
        ...
        FROM lease l
                 JOIN user_lease_tenant ult ON ult.lease_id = l.id AND ult.apartment_number IS NOT null
        WHERE l.site = ?
          AND SUBSTR(ult.semester, LOCATE(' ', ult.semester) + 1) >= (YEAR(CURDATE()) - 1)
        ORDER BY year, semester`;
