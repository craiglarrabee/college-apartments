...
export const DeleteUserLease = async (userId, leaseId) => {
    ...
    try {
        await Promise.all([
            ExecuteQuery("DELETE FROM user_lease WHERE lease_id=? AND user_id=?",
            [
                leaseId,
                userId
            ]),
            ExecuteQuery("DELETE FROM user_lease_tenant WHERE lease_id=? AND user_id=?",
            [
                leaseId,
                userId
            ])]);
...
export const GetActiveSemesterTenantNames = async (site) => {
    const query = `
        ...
        FROM lease l
                 JOIN user_lease_tenant ult ON ult.lease_id = l.id AND ult.semester IN (l.semester1, l.semester2) AND
