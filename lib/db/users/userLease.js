...
export const GetUserLeases = async (leaseDefinitionId, submitted) => {
    const queryString = submitted ?
        "SELECT l.id, l.lease_id, l.user_id, l.room_options_id, DATE_FORMAT(l.lease_date, '%M %d, %Y') AS lease_date, l.signature, DATE_FORMAT(l.signed_date, '%M %d, %Y') AS signature_date, l.lease_discount, l.vehicle_color, l.vehicle_make_model, l.vehicle_license, l.vehicle_state, l.vehicle_owner, l.lease_address, l.lease_home_phone, l.lease_cell_phone, l.lease_email, l.lease_parent_name, l.lease_parent_phone, CONCAT(t.first_name, ' ', t.last_name) AS name, a.room_type_id FROM user_lease l JOIN user_lease_tenant t on l.user_id = t.user_id JOIN application a on l.lease_id = a.lease_id WHERE l.lease_id = ? AND l.signed_date IS NOT NULL" :
        "SELECT l.id, l.lease_id, l.user_id, l.room_options_id, DATE_FORMAT(l.lease_date, '%M %d, %Y') AS lease_date, l.signature, DATE_FORMAT(l.signed_date, '%M %d, %Y') AS signature_date, l.lease_discount, l.vehicle_color, l.vehicle_make_model, l.vehicle_license, l.vehicle_state, l.vehicle_owner, l.lease_address, l.lease_home_phone, l.lease_cell_phone, l.lease_email, l.lease_parent_name, l.lease_parent_phone, CONCAT(t.first_name, ' ', t.last_name) AS name, a.room_type_id FROM user_lease l JOIN user_lease_tenant t on l.user_id = t.user_id JOIN application a on l.lease_id = a.lease_id WHERE l.lease_id = ? AND l.signed_date IS NULL";
    let [rows] = await ExecuteQuery(queryString, [leaseDefinitionId]);
    //we should only have one
    return rows;
}

export const AddUserLease = async (userId, leaseId) => {

    try {
        await ExecuteQuery("INSERT INTO user_lease (lease_id, user_id, room_options_id, lease_address, lease_home_phone, lease_cell_phone, lease_email, lease_parent_name, lease_parent_phone) SELECT a.lease_id, a.user_id, a.room_type_id, CONCAT(t.street,', ',t.city,', ',t.state,' ',t.zip), t.home_phone, t.cell_phone, t.email, t.parent_name, t.parent_phone FROM application a JOIN tenant t on a.user_id = t.user_id WHERE a.lease_id=? AND a.user_id=?",
            [
                leaseId,
                userId
            ]);
