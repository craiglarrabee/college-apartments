...
SELECT id, site, description, COALESCE(start_date, '') as start_date, COALESCE(end_date, '') as end_date, 
       semester1, semester2, deposit_amount
FROM lease 
WHERE id = ?`, [leaseId]);
...
export const GetLeases = async (site) => {
    const [rows] = await ExecuteQuery(`SELECT d.id, n.page, n.label 
        ...
        WHERE d.site = ? AND COALESCE(d.end_date, CURDATE() + interval 1 month) >= CURDATE()`,
        [site]);
    return rows;
}
...
export const UpdateLease = async (leaseDefinitionId, data) => {
    ...
    try {
        await ExecuteQuery(`
UPDATE lease 
SET description = ?, start_date = ?, end_date = ?, semester1 = ?, semester2 = ?, deposit_amount = ?
WHERE id = ?`,
            [
                data.description,
                data.start_date,
