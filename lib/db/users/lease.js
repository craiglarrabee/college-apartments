...
export const GetLease = async (leaseId) => {
    ...
    const [rows] = await ExecuteQuery("SELECT id, site, description, COALESCE(start_date, '') as start_date, COALESCE(end_date, '') as end_date, fall_year, spring_year, summer_year FROM lease WHERE id = ?", [leaseId]);
...
export const UpdateLease = async (leaseDefinitionId, data) => {
    ...
    try {
        await ExecuteQuery("UPDATE lease SET description = ?, start_date = ?, end_date = ?, fall_year = ?, spring_year = ?, summer_year = ? WHERE id = ?",
            [
                data.description,
                data.start_date,
                data.end_date,
