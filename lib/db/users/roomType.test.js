...
describe("UpdateLeaseRoom", () => {
    it("should update lease room", async () => {
        const data = {room_rent: 10, room_full: true};
        ...
        expect(ExecuteQuery).toHaveBeenCalledWith(
            ...
            [data.room_rent, data.room_full, "lease_id", "room_type_id"]
        );
    });
});
...
describe("CopyLeaseRooms", () => {
    it("should INSERT SELECT lease_rooms from source to target with correct params", async () => {
        ...
        expect(sql).toEqual(expect.stringContaining("SELECT site, room_type_id, ?, room_rent, room_full"));
