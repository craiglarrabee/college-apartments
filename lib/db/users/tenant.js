...
export const GetUserLeaseTenants = async (site, semester) => {
    const query = `
        ...
        WHERE a.processed = 1
          AND a.site = ?
             ${site === "snow" ? "AND a.deposit_date IS NOT NULL " : " "}
    ...
    const [rows] = await ExecuteQuery(`
                ...
                FROM user_lease_tenant ult1
                         INNER JOIN user_lease_tenant ult2
                                    ON ult2.semester = ult1.semester AND ult2.apartment_number = ult1.apartment_number AND ult2.selected = 1
                         INNER JOIN application a ON ult2.lease_id = a.lease_id AND ult2.user_id = a.user_id
                WHERE ult1.user_id = ?
                    ${ semester ? "AND ult1.semester = ?" : "" }
                                `,
        [
            userId,
            semester
...
export const UpdateUserLeaseTenant = async (userId, leaseId, data) => {
    ...
    try {
        await ExecuteQuery(`UPDATE user_lease_tenant SET last_name=?, first_name=?, middle_name=?, gender=?, 
                             date_of_birth=?, last_4_social=?, cell_phone=?, cell_phone2=?, home_phone=?, email=?, 
                             email2=?, convicted_crime=?, convicted_explain=?, charged_crime=?, charged_explain=?, 
                             street=?, city=?, state=?, zip=?, parent_name=?, parent_street=?, parent_city=?, 
