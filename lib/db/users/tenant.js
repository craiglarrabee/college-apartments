...
export const SearchTenantByName = async (searchStr) => {
    const query = `
        SELECT DISTINCT t.user_id, CONCAT(t.first_name, ' ', t.last_name) name
        FROM user_lease_tenant t
        WHERE MATCH(t.first_name, t.middle_name, t.last_name) AGAINST(?)
    `;
    let [rows] = await ExecuteQuery(query, [searchStr]);
    return rows;
}
export const GetTenant = async (id) => {

    let [rows] = await ExecuteQuery(`SELECT t.last_name,
                                            t.first_name,
                                            t.middle_name,
                                            t.gender,
...
export const GetUserLeaseTenants = async (site, year, semester) => {
    const query = `SELECT t.id,
                          ...
                          a.roomate_desc
                   FROM application a
                            INNER JOIN user_lease_tenant t ON a.user_id = t.user_id AND a.lease_id = t.lease_id AND
                                                              CASE ?
                                                                  WHEN 'fall' THEN t.fall_year
                                                                  WHEN 'spring' THEN t.spring_year
                                                                  WHEN 'summer' THEN t.summer_year END = ?
                            INNER JOIN room_type rt on a.site = rt.site AND a.room_type_id = rt.id
                   WHERE a.processed = 1
                     AND a.deposit_date IS NOT NULL
                     AND a.site = ?
...
export const GetUserRoomates = async (userId, year, semester) => {
    const [rows] = await ExecuteQuery(`SELECT CONCAT(t.first_name, ' ', t.last_name)                     AS name,
                                              t.apartment_number,
