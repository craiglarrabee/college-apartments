...
export const GetUserRoomates = async (userId, leaseId) => {
    const [rows] = await ExecuteQuery("SELECT CONCAT(t.first_name, ' ', t.last_name) AS name, t.apartment_number, CASE WHEN a.share_info = 1 THEN t.email ELSE NULL END AS email, CASE WHEN a.share_info = 1 THEN t.cell_phone ELSE NULL END AS cell_phone, CASE WHEN a.share_info = 1 THEN t.home_phone ELSE NULL END AS home_phone FROM user_lease_tenant t INNER JOIN application a on t.lease_id = a.lease_id AND t.user_id = a.user_id WHERE t.apartment_number IN (SELECT apartment_number FROM user_lease_tenant WHERE user_id = ? AND lease_id = ?)",
        [
            userId,
            leaseId
        ]);
