...
export const GetTenant = async (site, id) => {
    ...
    let [rows] = await ExecuteQuery(`
        SELECT t.last_name,
               ...
               u.username,
               sa.site_privs AS admin,
               sa.apartment_privs AS manage
        FROM tenant t
            INNER JOIN user u on t.user_id = u.id
            LEFT JOIN site_admins sa ON sa.userid = t.user_id AND sa.site = ?
            LEFT JOIN application a ON a.user_id = t.user_id AND a.processed = false AND a.site = ?
        WHERE t.user_id = ?`, [site, site, id]);
...
/**
 * Get Square customer ID for a tenant
 *
 * @param {number} userId - Tenant user ID
 * @returns {Promise<string|null>} Square customer ID or null if not set
 */
export const GetTenantSquareCustomerId = async (userId) => {
    try {
        const [rows] = await ExecuteQuery(
            `SELECT square_customer_id FROM tenant WHERE user_id = ?`,
            [userId]
        );
        return rows?.[0]?.square_customer_id || null;
    } catch (e) {
        console.error(`${new Date().toISOString()} - Error getting Square customer ID:`, e);
        throw e;
    }
};

/**
 * Update Square customer ID for a tenant
 *
 * @param {number} userId - Tenant user ID
 * @param {string} squareCustomerId - Square customer ID to save
 * @returns {Promise<void>}
 */
export const UpdateTenantSquareCustomerId = async (userId, squareCustomerId) => {
    try {
        await ExecuteQuery(
            `UPDATE tenant SET square_customer_id = ? WHERE user_id = ?`,
            [squareCustomerId, userId]
        );
        if (process.env.NODE_ENV !== 'production') {
            console.log(`${new Date().toISOString()} - Updated tenant ${userId} with Square customer ID: ${squareCustomerId}`);
        }
    } catch (e) {
        console.error(`${new Date().toISOString()} - Error updating Square customer ID:`, e);
        throw e;
    }
};
