...
describe('GetUserAndVerifyPassword', () => {
  ...
  it('should call ExecuteQuery with the correct query and arguments', async () => {
    const expectedQuery = 'SELECT user.password, user.id, tenant.first_name, site_admins.site FROM user LEFT JOIN tenant ON tenant.user_id = user.id LEFT JOIN site_admins ON site_admins.userid = user.id and site_admins.site = ? WHERE username = ?';
...
describe('ChangeUserPassword', () => {
  ...
  it('should call GetUserAndVerifyPassword with the correct arguments', async () => {
    ...
    expect(ExecuteQuery).toHaveBeenCalledWith("SELECT user.password, user.id, tenant.first_name, site_admins.site FROM user LEFT JOIN tenant ON tenant.user_id = user.id LEFT JOIN site_admins ON site_admins.userid = user.id and site_admins.site = ? WHERE username = ?", ["site1", "user1"]);
  });
