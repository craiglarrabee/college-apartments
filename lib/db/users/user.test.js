...
describe("GetUserAndVerifyPassword", () => {
    ...
    it("should call ExecuteQuery with the correct query and arguments", async () => {
        const expectedQuery = "SELECT user.password, user.id, tenant.first_name, tenant.email FROM user LEFT JOIN tenant ON tenant.user_id = user.id WHERE username = ?";
        const expectedArgs = ["user1"];
        ExecuteQuery.mockResolvedValue([[{}]]);
        await GetUserAndVerifyPassword("user1", "p@ssw0rd");
    ...
    it("should throw an error if query returns no rows", async () => {
        ExecuteQuery.mockResolvedValue([[]]);
        await expect(GetUserAndVerifyPassword("user1", "p@ssw0rd")).rejects.toThrow("invalid login");
    });
...
describe("ChangeUserPassword", () => {
    ...
    it("should call initial SELECT to verify current password with the correct query and arguments", async () => {
        ExecuteQuery.mockResolvedValue([[mockUserData]]);
        await ChangeUserPassword("site1", "user1", "p@ssw0rd", "new_p@ssw0rd");
        expect(ExecuteQuery).toHaveBeenCalledWith(
            "SELECT user.password, user.id, tenant.first_name, tenant.email FROM user LEFT JOIN tenant ON tenant.user_id = user.id WHERE username = ?",
            ["user1"]
        );
    });

    it("should throw an error if GetUserAndVerifyPassword returns no user data", async () => {
        // Simulate no rows returned from the initial SELECT
