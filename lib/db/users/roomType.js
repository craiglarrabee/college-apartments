...
export const GetLeaseRooms = async(leaseId) => {
    ...
    const [rows] = await ExecuteQuery("SELECT * FROM lease_rooms lr JOIN room_type rt on rt.id = lr.room_type_id AND rt.site = lr.site WHERE lr.lease_id = ? ORDER BY lr.lease_id, lr.room_rent ",
        [
            leaseId
        ]);
        return rows;
