...
export const GetLeaseRooms = async (leaseId) => {
    ...
    const [rows] = await ExecuteQuery(`SELECT DISTINCT l.description, lr.*, rt.*, l.deposit_amount
                                       FROM lease_rooms lr
                                                JOIN room_type rt on rt.id = lr.room_type_id AND rt.site = lr.site
                                                JOIN lease l on l.id = lr.lease_id and l.site = lr.site
                                                JOIN apartment a on rt.base_type_id = a.room_type_id AND a.site = rt.site
                                       WHERE lr.lease_id = ?
...
export const GetVisibleSemesterLeaseRoomTypes = async (site, semester) => {
    const query = `
    SELECT l.id lease_id, l.description, lr.room_type_id, lr.room_rent, lr.room_full, rt.base_type_id, rt.room_desc, rt.room_type, rt.location
    FROM lease l
...
export const GetUserAvailableLeaseRooms = async (site, userId) => {
    ...
    if (site === "suu") {
        ...
        query = `SELECT DISTINCT l.description, rt.room_desc, lr.lease_id, lr.room_type_id, lr.room_rent, l.deposit_amount, rt.location
                                       FROM lease_rooms lr
                                                 JOIN room_type rt on rt.id = lr.room_type_id AND rt.site = lr.site
                                                 JOIN lease l on l.id = lr.lease_id and l.site = lr.site
                                                 JOIN apartment a on rt.base_type_id = a.room_type_id AND a.site = rt.site
                                       WHERE l.site = ?
                                         AND coalesce(lr.room_full,0) = 0
        ...
        query = `SELECT DISTINCT l.description, rt.room_desc, lr.lease_id, lr.room_type_id, lr.room_rent, l.deposit_amount, rt.location
                                       FROM lease_rooms lr
                                                 JOIN room_type rt on rt.id = lr.room_type_id AND rt.site = lr.site
                                                 JOIN lease l on l.id = lr.lease_id and l.site = lr.site
                                                 JOIN apartment a on rt.base_type_id = a.room_type_id AND a.site = rt.site AND a.room_type_id
                                       WHERE l.site = ?
                                         AND coalesce(lr.room_full,0) = 0
...
export const UpdateLeaseRoom = async (leaseId, roomTypeId, data) => {
    ...
    try {
        await ExecuteQuery("UPDATE lease_rooms SET room_rent = ?, room_full = ? WHERE lease_id = ? AND room_type_id = ? ",
            [
                data.room_rent,
                data.room_full,
                leaseId,
