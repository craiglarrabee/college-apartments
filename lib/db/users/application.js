...
export const GetUnprocessedApplications = async (site, leaseId) => {
    const [rows] = await ExecuteQuery("SELECT t.user_id, CONCAT(t.first_name, ' ', t.last_name) as name, DATE_FORMAT(a.submit_date, '%M %d, %Y') AS submit_date FROM application a INNER JOIN tenant t ON a.user_id = t.user_id WHERE a.site = ? AND a.lease_id = ? AND a.processed = 0 AND a.deposit_date IS NULL", [site, leaseId]);
    return rows;
}

export const GetProcessedApplications = async (site, leaseId) => {
    const [rows] = await ExecuteQuery("SELECT t.user_id, CONCAT(t.first_name, ' ', t.last_name) as name, DATE_FORMAT(a.submit_date, '%M %d, %Y') AS submit_date FROM application a INNER JOIN tenant t ON a.user_id = t.user_id WHERE a.site = ? AND a.lease_id = ? AND a.processed = 1 AND a.deposit_date IS NULL", [site, leaseId]);
    return rows;
}

export const GetDepositReceivedApplications = async (site, leaseId) => {
    const [rows] = await ExecuteQuery("SELECT t.user_id, CONCAT(t.first_name, ' ', t.last_name) as name, DATE_FORMAT(a.submit_date, '%M %d, %Y') AS submit_date, DATE_FORMAT(a.deposit_date, '%M %d, %Y') AS deposit_date FROM application a INNER JOIN tenant t ON a.user_id = t.user_id WHERE a.site = ? AND a.lease_id = ?  AND a.deposit_date IS NOT NULL", [site, leaseId]);
    return rows;
}
