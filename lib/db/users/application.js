...
export const GetApplications = async (site, leaseId) => {
    const [rows] = await ExecuteQuery("SELECT t.user_id, CONCAT(t.first_name, ' ', t.last_name) as name, t.email, DATE_FORMAT(a.submit_date, '%M %d, %Y') AS submit_date, DATE_FORMAT(a.deposit_date, '%M %d, %Y') AS deposit_date, a.processed, t.apartment_number, t.lease_id, DATE_FORMAT(l.lease_date, '%M %d, %Y') AS lease_date, ld.spring_year AS lease_spring_year, ld.summer_year AS lease_summer_year, ld.fall_year AS lease_fall_year, t.spring_year, t.summer_year, t.fall_year FROM application a INNER JOIN lease ld ON ld.id = a.lease_id INNER JOIN user_lease_tenant t ON a.user_id = t.user_id AND a.lease_id = t.lease_id LEFT JOIN user_lease l ON l.user_id = t.user_id AND l.lease_id = t.lease_id WHERE a.site = ? AND a.lease_id = ? ",
        [
            site,
            leaseId
        ]);
