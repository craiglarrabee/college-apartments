...
export const GetApplications = async (site, leaseId) => {
    const [rows] = await ExecuteQuery("SELECT t.user_id, CONCAT(t.first_name, ' ', t.last_name) as name, t.email, DATE_FORMAT(a.submit_date, '%M %d, %Y') AS submit_date, DATE_FORMAT(a.deposit_date, '%M %d, %Y') AS deposit_date, a.processed, t.apartment_number, t.lease_id FROM application a INNER JOIN user_lease_tenant t ON a.user_id = t.user_id AND a.lease_id = t.lease_id WHERE a.site = ? AND a.lease_id = ? AND NOT EXISTS(SELECT * FROM user_lease WHERE user_id = t.user_id AND lease_id = t.lease_id)",
        [
            site,
            leaseId
        ]);
