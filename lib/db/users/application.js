...
export const ReceiveDeposit = async (site, userId, leaseId) => {
    try {
        console.log(`${new Date().toISOString()} - Starting ReceiveDeposit for User: ${userId}, Lease: ${leaseId}, Site: ${site}`);
        await ExecuteQuery("UPDATE application SET deposit_date = NOW(), processed = 1 WHERE site=? AND user_id=? AND lease_id=?",
            [
                site,
                userId,
                leaseId,
            ]);
        await AddUserLeaseTenant(userId, leaseId);
        const [rows] = await ExecuteQuery("SELECT t.user_id, CONCAT(t.last_name, ', ', t.first_name) as name, DATE_FORMAT(a.submit_date, '%M %d, %Y') AS submit_date, DATE_FORMAT(a.deposit_date, '%M %d, %Y') AS deposit_date, processed, l.deposit_amount, rt.location FROM application a INNER JOIN tenant t ON a.user_id = t.user_id INNER JOIN lease l ON a.lease_id = l.id INNER JOIN room_type rt ON a.room_type_id = rt.id AND a.site = rt.site WHERE a.site = ? AND a.lease_id = ? and a.user_id = ?",
            [
                site,
                leaseId,
                userId
            ]);
        console.log(`${new Date().toISOString()} - Finished ReceiveDeposit for User: ${userId}. Success: ${!!rows[0]}`);
        return rows[0];
    } catch (e) {
        console.error(`${new Date().toISOString()} - Error in ReceiveDeposit:` , e);
    }
};
...
export const IsReturningStudent = async (userId, site, targetLeaseId) => {
    ...
    if (site === 'suu' && targetLeaseId) {
        ...
        if (semester1?.startsWith('Fall') || semester2?.startsWith('Fall')) {
            ...
            `;
            params = [userId, site, summerSemester, summerSemester];
        } else {
...
export const GetApplications = async (site, leaseId) => {
    const query = `
        ...
        GROUP BY a.lease_id, ult.user_id, ult.date_of_birth, a.room_type_id, a.submit_date,
                 a.deposit_date, a.processed, ult.apartment_number, ul.lease_date, ul.signed_date,
...
export const GetTenantApplications = async (site, userId) => {
    ...
    let [rows] = await ExecuteQuery(`SELECT a.site,
                                            ...
                                            a.clean_work,
                                            a.accepted,
                                            COALESCE(n.label, l.description)         AS label,
                                            l.deposit_amount
                                     FROM application a
                                              LEFT JOIN room_type rt on a.room_type_id = rt.id AND a.site = rt.site
                                              LEFT JOIN lease l on a.lease_id = l.id AND a.site = l.site
                                              LEFT JOIN site_nav n on n.page = CONCAT('leases/', l.id) AND a.site = n.site
                                     WHERE a.site = ?
                                       AND a.user_id = ?
    `, [site, userId]);
    return rows;
