...
export const AddApplication = async (site, userId, leaseId, data) => {
    await ExecuteTransaction([
        ...
        },
        {
            ...
            string: `
                INSERT INTO user_required_payments (user_id, lease_id, payment_number, payment_type, amount, semester)
                SELECT DISTINCT ult.user_id, ult.lease_id, 0, 'deposit', l.deposit_amount, GROUP_CONCAT(ult.semester SEPARATOR '/') semester
                FROM user_lease_tenant ult
                         JOIN lease l on l.id = ult.lease_id
                WHERE ult.user_id = ?
                  AND ult.lease_id = ?
            `, params: [
                userId,
