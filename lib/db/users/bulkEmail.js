...
export const AddEmailDefinition = async (message_id, data) => {
    ...
    await ExecuteQuery(`INSERT INTO email_definition (message_id, from_address, subject, body, site, semester)
                        VALUES (?, ?, ?, ?, ?, ?)`,
        [
            message_id,
            data.from,
...
export const GetBulkEmailDetails = async (messageId) => {
    const query = `SELECT DISTINCT ed.message_id,
                   ...
                   FROM email_definition ed
                       ...
                       AND CASE
                               WHEN ed.semester LIKE 'Fall%' THEN t.fall_year = SUBSTR(ed.semester, 6, 4)
