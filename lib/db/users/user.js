...
export const GetUserAndVerifyPassword = async (site, username, password) => {
    const [rows] = await ExecuteQuery("SELECT user.password, user.id, tenant.first_name, tenant.email, site_admins.site FROM user LEFT JOIN tenant ON tenant.user_id = user.id LEFT JOIN site_admins ON site_admins.userid = user.id and site_admins.site = ? WHERE username = ?",
        [
            site,
            username
        ]);
