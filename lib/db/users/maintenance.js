...
export const GetAllMaintenanceSemesters = async (site) => {
    const query = `
        ...
        FROM (
            SELECT semester FROM maintenance_request WHERE site = ? AND semester IS NOT NULL AND semester != ''
            UNION
            SELECT ult.semester 
            FROM user_lease_tenant ult
            JOIN lease l ON l.id = ult.lease_id
            WHERE l.site = ? AND ult.semester IS NOT NULL AND ult.semester != ''
              AND SUBSTR(ult.semester, LOCATE(' ', ult.semester) + 1) >= (YEAR(CURDATE()) - 1)
        ) as combined_semesters
        ORDER BY SUBSTR(semester, LOCATE(' ', semester) + 1) DESC, 
                 ...
                 END DESC
    `;
    const [rows] = await ExecuteQuery(query, [site, site]);
    return rows || [];
};
