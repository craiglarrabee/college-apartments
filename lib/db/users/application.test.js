...
describe("GetApplicationInfo", () => {
    ...
    it("should call ExecuteQuery with the site, userId, leaseId, and roomTypeId", async () => {
        ...
        expect(sql).toContain("a.site = ?");
        expect(sql).toContain("a.user_id = ?");
        expect(sql).toContain("a.lease_id = ?");
        expect(sql).toContain("a.room_type_id = ?");
...
describe("ReceiveDepositInfo", () => {
    afterEach(() => {
        jest.clearAllMocks();
    });

    it("should update application with deposit_date and processed = 1 and call AddUserLeaseTenant", async () => {
        const site = "fakeSite";
        const userId = "fakeId";
        const leaseId = "fakeLeaseId";
        
        ExecuteQuery.mockResolvedValueOnce([]); // UPDATE query
        ExecuteQuery.mockResolvedValueOnce([[{user_id: userId, processed: 1, deposit_amount: 100, location: "Park Place"}]]); // SELECT query
        
        const result = await ReceiveDeposit(site, userId, leaseId);
        
        expect(ExecuteQuery).toHaveBeenCalledWith(
            expect.stringContaining("UPDATE application SET deposit_date = NOW(), processed = 1"),
            [site, userId, leaseId]
        );
        
        expect(ExecuteQuery).toHaveBeenLastCalledWith(
            expect.stringContaining("rt.location"),
            [site, leaseId, userId]
        );
        
        expect(result.location).toBe("Park Place");
