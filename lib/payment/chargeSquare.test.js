...
describe('chargeSquare', () => {
  ...
  describe('Card Declined Scenarios', () => {
    ...
    test('handles CARD_DECLINED with no detail (code only)', async () => {
      __mock.paymentsApi.createPayment.mockRejectedValue(
        new ApiError([{ code: 'CARD_DECLINED' }])
      );

      await expect(
        chargeSquare({ ...basePaymentRequest, squareSourceId: 'cnon:card-declined-no-detail' })
      ).rejects.toMatchObject({
        statusCode: 400,
        errormessage: 'Your card was declined. Please try a different payment method or contact your card issuer.'
      });
    });

    test('handles GENERIC_DECLINE with no detail', async () => {
      __mock.paymentsApi.createPayment.mockRejectedValue(
        new ApiError([{ code: 'GENERIC_DECLINE' }])
      );

      await expect(
        chargeSquare({ ...basePaymentRequest, squareSourceId: 'cnon:generic-decline' })
      ).rejects.toMatchObject({
        statusCode: 400,
        errormessage: 'Your card was declined. Please try a different payment method or contact your card issuer.'
      });
    ...
    test('handles INSUFFICIENT_FUNDS with no detail', async () => {
      __mock.paymentsApi.createPayment.mockRejectedValue(
        new ApiError([{ code: 'INSUFFICIENT_FUNDS' }])
      );

      await expect(
        chargeSquare({ ...basePaymentRequest, squareSourceId: 'cnon:insufficient-funds-no-detail' })
      ).rejects.toMatchObject({
        statusCode: 400,
  ...
  describe('Order and Payment Response Handling', () => {
    ...
    test('handles unmapped error code (falls back to generic message)', async () => {
      __mock.paymentsApi.createPayment.mockRejectedValue(
        new ApiError([{ code: 'UNKNOWN_ERROR_CODE' }])
      );
      ...
      await expect(
        ...
        statusCode: 400,
        errormessage: 'The payment could not be processed. Please try a different payment method or contact support.'
      });
    });
